...
const soundUrl = {
  ...
  handsClapping: 'hands_clapping.mp3',
  error: 'error_mix.mp3'
}
const animationCongra = require('../assets/lottieJSON/thong20.json')
const animationError = require('../assets/lottieJSON/error.json')

const consoleLog = n => console.log('=== Puzzle.js - line: ' + n + ' ================================')
...
function Puzzle(){
  ...
  const showAlert = (DATA) => {
    ...
    if (count === 15) {
      _playSound(soundUrl.handsClapping)
  ...
  const _renderItem = ({ item, index }) => {
    if (item === "") {
      ...
      return (
        <TouchableOpacity
          style={styles.cell}
          onPress={() => _handlerTouch(item, index, _playSound(soundUrl.click))}
          // onPress={() => _handlerTouch(item, index)}
        >
